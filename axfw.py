...
''', formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    config_group.add_argument("-f", "--file", help='aXiom firmware file (.alc or .axfw format)',
                              metavar='FIRMWARE_FILE')
    config_group.add_argument("--info",
                              help='Displays the firmware information of the connected aXiom device and the firmware files (.axfw only)',
                              action="store_true")
    ...
    if args.info:
        if args.file is None:
            ...
            return_code, device_id, fw_variant, fw_ver_major, fw_ver_minor, fw_ver_patch, fw_status, _ = axfw_get_fw_info_from_file(
                args.file)
            ...
            if return_code == 0 or (return_code in [7, 8] and args.force):
                ...
                if return_code == 0:
                    ...
                    if u33.reg_runtime_nvm_crc != fw_crc:
                        print(
                            f"ERROR: Firmware CRC check failed. Device: 0x{u33.reg_runtime_nvm_crc:08X}, File: 0x{fw_crc:08X}")
                        return_code = 9
            elif return_code == 5:
                print("ERROR: Unknown .axfw file")
            elif return_code == 7:
